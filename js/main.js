...
if (session === "login" && username) {
    ...
    .then(data => {
        if (typeof data.score === "number") {
            i = data.score;
            game.score = i; // <--- Corrigido: atualiza game.score
        ...
        if (data.professores_comprados && typeof data.professores_comprados === "object") {
            ...
            game.professores = professoresComprados; // <--- Atualiza game.professores
            localStorage.setItem('professores_comprados', JSON.stringify(professoresComprados));
            // Atualiza bonus para refletir último professor comprado
        ...
        load(); // Atualiza tela e animações
    })
    .catch(() => {
        load();
    });
